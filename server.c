...
static void handle_get (int connection_fd, const char* page)
{
 ...
 if (*page == '/' && strchr (page + 1, '/') == NULL) {
    ...
    page_aux = malloc(sizeof (page));
    req_params = "";
    strcpy(page_aux, page);
    if (strchr(page_aux, '?') != NULL) {  // If URL has parameters (query parameter char)
      char *aux;

      aux = malloc(sizeof (page));
      strcpy(aux, page);
      strtok(page_aux, "?"); // Spliting string by ? character
      snprintf(module_file_name, sizeof (module_file_name),
              "%s.so", page_aux + 1);

      aux = strchr(aux, '?'); // search for ? character in aux
      req_params = aux + 1;

      module = module_open(module_file_name);
    } 
    else { // Without parameters, normal run
      snprintf (module_file_name, sizeof (module_file_name),
          "%s.so", page + 1);
      module = module_open (module_file_name);
    }
